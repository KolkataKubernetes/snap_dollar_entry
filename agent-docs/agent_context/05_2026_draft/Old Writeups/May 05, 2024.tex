...
\textbf{Objective.} (1) Provide evidence of cannibalization, by showing that incremental sales decrease as the number of store openings increases in a state. (2) Provide upper and lower bounds for the profit function's primitives, using as an instrument the ``swaps" that represent the trade-off between proximity to benefits and distance to warehouses.

\textbf{Contribution.} Provide insights on the quick entry of dollar stores and providing some preliminary evidence regarding whether SNAP benefits might impose a trade-off between distance to distribution centers and proximity to SNAP beneficiaries.

\textbf{Motivation.} During the last decades, dollar stores have expanded significantly, more than other food retailers. Understanding their rapid expansion is a matter of public policy, as these stores disproportionately cater to low-income households, and in some instances, they are the only firm market. Anecdotally, their fast expansion has also been linked to health hazards\footnote{In February 2024, Dollar Tree stores were selling recalled applesauce contaminated with lead. In the same month, Family Dollar warehouses were experiencing rodent infestations. Previously, in October 2023, Dollar General was deemed a Severe Violator of OSHA by the Department of Labor.}. 
...
\textbf{Method Overview. } The method has two stages. The first one is a descriptive to make sure that changes in the snap benefits are correlated with greater entry of dollar stores. The second one is to model the expenditure in dollar stores, and link this to a model of dollar store network expansion, in a similar fashion as Holmes \citeyear{holmes2011diffusion} and Houde, Newberry and Seim \citeyear{houde2023nexus}. 


\textbf{Descriptive Evidence Approach.} The first step is to provide descriptive a analysis on how the changes in dollar stores affect their entry. To this extent, it ideal (1) to do a differences in differences analysis the dollar stores entry is correlated with increases in the snap benefits, (2) and do a event study analysis to make sure that there there is not a timing issue. That is, a concern is that as a dollar store opens then consumers are more likely to enroll into SNAP, to maximize their consumption. The differences in differences approach would go as follows. The entry of a dollar store $j$ in a market $m$ at time $t$:
...
\textit{Supply Side.}  An infinitely living {Dollar Store} is endowed at $t=0$ with a network $N_0$, and with a exogenous stock of warehouses, faces a profit flow of:
       ...
       Before fully estimating the model, one can provide evidence the cannibalization and economies of densities by measuring the incremental sales. That is, measure the difference between the observed sales, and the sales if the store did not open. For this I will make use  of the demand side.  The full identification of this model would rely on moment inequalities.As in \citeauthor{holmes2011diffusion} (2010) I will use \citeauthor{bajari2005measuring} (2005) and \citeauthor{fox2007semiparametric} (2007) analysis of \textit{pairwise re-sequenced} inequalities. Namely, one would consider counterfactual networks composed by swapping the 
       the opening of a store $j$ with the one of another store $j'$. By defining such counterfactual sequence as $N^{jj'}$, from the observed one $N^*$, the implied inequality is:

      \[  m_N(\theta) \equiv&\Pi(N^*; \theta)  - \Pi(N^{j,j'};\theta)  + \epsilon^{jj'} \geq 0 \]

      Where $\epsilon^{jj'}$ is a measurement error. Now, one might be concerned that the error is correlated with the revenue and cost variables. To address this it is useful to incorporate an instrumental variable that that is not related to the error, that is able to capture the trade-offs associated to the firm maximization problem. In this instance, the objective would be to use as instrumental functions swaps that involve an increase in exposure to SNAP benefits, and an increase in cost distance to warehouses. Formally $Z^{jj'}_1 = \mathbf{1}\{\Delta SNAP^{jj'} >0, C_d^{jj'} >0\}$ and $Z^{jj'}_2 = \mathbf{1}\{\Delta SNAP^{jj'} <0,  C_d^{jj'} <0\}$. Under this structure if we define $Y^{j,j'}$ as the difference in gross profits between the observed outcome and the counterfactual outcome, and we consider that the other costs are ``fixed" then one could retrieve the upper and lower bounds of the distance primitives $\theta_d$ using the instruments:
     ...
     This bounds the primitive, but also serves as evidence for the importance of the SNAP benefits. If there is a trade-off between distance to warehouses and the exposure to snap benefits, one would expect change in expected gross revenues  to go up as the distance goes up ($\mathbf{E}[Y^{jj'} \mid Z^{1}_{jj'}]<0$), and to go down as the distance goes down  ($\mathbf{E}[Y^{jj'} \mid Z^{2}_{jj'}]<0$). 
      

% The demand side in the Holmes article relies on a  maximum likelihood estimator. Holmes observes the sales of each store and provides a model of Walmart Store, and his model's demand side relies on a discrete choice model of Walmarts based on the observable characteristics of the Walmarts. With this discrete choice the revenue of a store $j$:

% \[\hat R_{j}(\theta, \lambda) = \lambda \times \sum_{\ell: j \in \mathcal{B}_\ell}  s_{\ell j }(\theta) \times n_\ell \Rightarrow R_j^\text{data} =  \hat R_j(\theta, \lambda)  +  \eta_{j} \quad \eta_j \sim \mathcal{N}(0,1)  \]

% Where $\lambda$ is the average expenditure for each consumer. Provided the error structure, he can estimate this using maximum likelihood. 

% Houde, Newberry and Seim (HNS), on the other hand rely on a reduced form from the minimum expenditure function of a household with CES preferences over varieties. In contrast with the Amazon setting, the distance to stores do matter. 

% Provided a swap of stores $(j,j')$, will be dominated by the observed preference. Under the instrument begin $Z=\mathbf{1}\{\Delta X_d^{(j,j')} > 0, \Delta \text{SNAP}^{(j,j')} <0\}$

% \[\mathbb{E}[\Delta Y^{(j,j')} - \theta_d \Delta X_d^{(j,j')} \mid Z] \geq 0  \iff  \frac{\mathbb{E}[\Delta Y^{(j,j')} \mid Z]}{\mathbb{E}[\Delta X^{(j,j')}\mid Z]} \leq \theta_d \]

% And the lower bound of the estimate is achieved with, $Z=\mathbf{1}\{\Delta X_d^{(j,j')} < 0, \Delta \text{SNAP}^{(j,j')} >0\}$. If the instrument successfully capture the trade-off between distance and profits, changes in profits and shipping distance should be positively correlated with the changes in the profit. 
