...
% \section{A toy model for inefficiencies}

% An issue is that healthy consumption invoques an intertemporal choice. Say that there is a two period model, an agent decides what to consume today which will affect its health tomorrow. There are poor and rich households. Consuming healthy will reduce the likelihood of illness in the second stage, but it is not deterministic. Something as:

% \[V(s) = \max_a u(a, h(s)) + \beta \mathbb{E}_s[V(s') \mid s, a ]  + \varepsilon(a,s) \quad a = \{H,U\} \]

% The conditional choice probability is going to be the logit share:

% \[Pr(a'=a \mid s ) = \frac{\exp \{u(a, h(s)) + \beta \mathbb{E}_s[V(s') \mid s, a] \}}{\sum_{a'} \exp\{u(a', h(s)) + \beta \mathbb{E}_s[V(s') \mid s, a'] \}}\]

% To arise inefficiencies it would have to be that households underestimate the implications of their decisions. Alternatively, one can claim that there is an inefficiency in the use of the resources directed to SNAP, if they are not being used for their intended purpose. From the policy perspective the inefficiency falls in a moral hazard setting, where the resources are provided but the lack of a well defined group of elegible goods allow for the expenditure on potentially unhealthy options. 

% In its most dramatic scenario, dollar store entry will be inefficient if the health implications outweigh the utility from lower prices, and the firm profits from entry. SNAP's objective will also be dampened if its implementation displaces healthy alternatives. The problem of making this claim in the model is that it would require a full model that incorporates today consumption and tomorrow's health outcomes. 



% \section{Questions}

% \textbf{Question 1.} Should I prioritize a full-blown reduced-form version of the paper? Or should I do this as a side-project? 

% \textbf{Question 2. }Should I pursue a further structural approach? I think that going over it from a competition perspective is difficult as there are papers that are doing a good job around it with really good data. Would it be believable to infer sales from consumer panel data (homescan)? If so, I could use homescan to (1) model competition across stores, (2) distinguish consumption between healthy-unhealthy goods. Keeping the same structure as in Grieco:

% \begin{enumerate}
%     \item Household has a unit of expenditure and decides to allocate it between healthy or unhealthy consumption.
%     \item After deciding where is this unit allocated to, it directs it to a particular store (all available options, not only dollar stores). The errors are correlated at the store-brand level. 
%     \item Revenues from store $s$, in market $m$ at time $t$ would be:

%     \[R_{smt} = \int_{i \in I_{mt}} \text{Weight}_{it} \times \text{Income}_{it} \times \underbrace{\alpha}_{\text{Grocery Budget Share (parameter)}} \times \left(\sigma_{ist}^{\text{Healthy}} (\cdot) + \sigma_{ist}^{\text{Non-Healthy}} (\cdot) \right) dF_{it}  \]
% \end{enumerate}

% For the scaling, one would have to use the projection weight and hope that the sampling is representative at the market level. 

% \textbf{Question 3.} When the dollar-store/convenience store enters, it doesn't internalize that the displacement of healthy offers will affect the health of the consumers. As pointed out too, another approach is from a belief perspective where the households have incorrect beliefs about the implications of unhealthy consumption. \underline{Is this a stretch?} A model of unhealthy consumption as in Barahona, Otero, Otero (BOO; ECMA, 2023) or Corradini (2024) the agent doesn't know the impacts that consumption has on health, and will base its expectation on its information set. In the BOO the policy was the implementation of labels (pre-post), on Corradini's the information comes from a prior and a quality signal (if avaialable):

% %This is outside of the paper. Competition outlook: 

% \[\mathbb{E}[u_{ijt}|I_{ijt}] =  \underbrace{\delta_{ijt}}_{\text{Stochastic Experience Utility}} - \underbrace{\alpha_i p_{jt}}_{\text{Price Desutility}} - \underbrace{\mathbb{E}[w_{jt}'\mid I_{jt}] \phi_i}_{\text{Health Cost}} \]


% Both articles exploit the availability of the information. Potential information changes:

% \textit{1. Changes in the labels}. In \textbf{2020} the labeling requirements changed, but they are very succinct. 
% \begin{figure}[h!]
%     \centering
...
% Required step: show that consumers react to the change in labels. Interestingly, firms have been encouraged to put front of package labels (as in BOO's Chile implementation, but less implicit). 


% \textit{2. Questionable Information.} In early 2009 a group of food producers and retailers launched a front of package information to recognize certain foods as \textit{Smart Choices}. However, the program was labeling sugary food as a healthy alternative, generating outcry, and its end in October of the very same year.


% \textbf{Question 4.} From the current perspective of the article, there is also an inefficiency that arises from a moral hazard perspective. The policy maker provides a subsidy, which in paper has the intention to improve the nutritional outcomes of certain households. As there is no monitoring on the expenditure, the incentives of the household of what to purchase not necessarily align with the policy maker decision. This idea is connected with the debate and related pilots where the SNAP incentive is augmented with rebates associated to the consumption of healthy food. A potential counterfactual would assess the effectiveness of the policy on a larger scale. Is this a viable argument? I am not aware of articles that talk about the moral hazard of policies.
