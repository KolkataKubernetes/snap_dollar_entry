...
Status: Complete (Ingest Complete Through 2019; Further Tract Descriptives Deferred)  
...
| 2026-03-21 | Executed the revised Milestone 2 rerun with the explicit tract exclusion set, `B20002_001E` tract income, and `B25064_001E` tract rent, then recorded the sidecar diagnostics before the later decision to curtail the retained tract branch at `2019` | Codex |
...
| 2026-03-21 | Closed the ExecPlan after confirming the tract branch is complete through the `2019`-bounded ingest sidecars, documenting the initial tract descriptive outputs and the waiver-descriptive source correction, and deferring further tract descriptive interrogation to a separate future ExecPlan | Codex |
...
The completed work adds tract-specific ingest scripts, tract-specific processed analysis artifacts, and two initial tract companion descriptive scripts. The observable deliverables are a tract waiver panel, a tract analysis panel before and after covariate matching, and initial tract descriptive outputs written into the existing descriptive output folders with `_tract` in their filenames and user-facing titles. Broader tract descriptive interrogation is deferred to a separate future ExecPlan.
...
- [x] (2026-03-21 America/Chicago) Rewrote the Milestone 2 tract ACS script so it pulls tract ACS 5-year estimates for every retained year `2010:2019`, writes review-stage annual-ACS sidecar artifacts, and builds year-specific tract controls for those years plus a documented `2010` backfill for `2000:2009`.
- [x] (2026-03-21 America/Chicago) Rewrote the Milestone 2 tract final-panel builder so it merges the year-specific tract ACS controls onto the tract panel, backfills tract ACS slots in `2000:2009` with the `2010` ACS 5-year values to preserve the `2000:2019` county-style year grid, and writes review-stage sidecar panel summaries for user inspection before any canonical overwrite.
...
- [x] (2026-03-21 America/Chicago) Closed the current ExecPlan after the user chose to defer further tract descriptive interrogation to a separate future plan. The tract branch is complete here through the `2019`-bounded ingest sidecars plus the two initial tract descriptive outputs already created.
...
- Observation: The current tract ACS implementation uses the `2010` ACS 5-year tract vintage as a time-invariant control repeated across the full tract panel, but the user now wants tract ACS controls to vary by year from `2010` onward.
  Evidence: `1_0_1_4_ACS_tract_prep.R` originally expanded a single `tract_covariates_2010` table across the full panel year grid, and the user explicitly requested that tract year `t` use the tract ACS 5-year estimate for year `t` for all retained ACS years from `2010` onward.
...
- Observation: Before the retained tract branch was curtailed to `2019`, the explicit-exclusion rerun resolved the reviewed `2011:2019` mismatch problem for non-excluded tracts while still exposing a larger `2020` tract-universe mismatch.
  Evidence: the refreshed `2_1_10_acs_tract_2010_2020_match_summary.rds` first reported `26` explicit exclusions in every ACS year, `73031` joined non-excluded scope tracts and `0` unjoined non-excluded scope tracts for every year `2010:2019`, and then `61620` joined non-excluded scope tracts with `11411` unjoined non-excluded scope tracts in `2020`; the later retained branch removed `2020` entirely.
...
- Observation: The revised Milestone 2 rerun materially reduced tract-panel covariate missingness relative to the earlier ratio-based annual sidecars.
  Evidence: the refreshed `2_9_7_us_analysis_panel_tract_timevarying_summary.rds` reports `1460620` rows, `73031` included tract units, `26` excluded tract units, `15330` missing tract income rows, and `38630` missing tract rent rows after switching to `B20002_001E`, `B25064_001E`, and curtailing the sidecars to `2019`.
...
- Observation: The review-stage annual-ACS tract summary makes the `2010` backfill visible and shows that the benchmark county wage gap remains confined to pre-2010 years.
  Evidence: `2_9_7_us_analysis_panel_tract_timevarying_summary.rds` reports `730310` `pre_2010_backfill_rows`, `730310` `post_2010_timevarying_rows`, `292124` missing `wage` and `wage_st`, `292124` missing wages before `2010`, and `0` missing wages in `2010:2019`.

- Observation: Tract-level `DP03_0063E` still appears unusable in this branch.
  Evidence: the annual sidecar match summary `2_1_10_acs_tract_2010_2020_match_summary.rds` reports `dp03_0063_available_tracts = 0` for every retained ACS year `2010:2019`, matching the earlier single-vintage tract result.
...
- Observation: The first tract and county waiver-count descriptives were flat for a mechanical plotting reason, not because treatment was constant.
  Evidence: both waiver descriptive scripts initially used `count(..., wt = 1, ...)`, which forces every grouped count to equal `1`; removing that argument corrected the plotting logic.

- Observation: The tract analysis panel is not the right source for waiver-timing descriptives.
  Evidence: `1_0_2_1_build_analysis_panel_tract_pre_covariates.R` inherits the legacy line `treatment = if_else(year %in% 2010:2013, 1L, treatment)`, so tract waiver descriptives should read the tract waiver source directly rather than count `treatment` from the tract panel.
...
- Decision: In the revised tract ACS branch, preserve the full `2000:2019` tract-year grid by backfilling tract ACS controls in years `2000:2009` with the `2010` ACS 5-year tract values.
  Rationale: The user wants the tract branch to match the county workflow’s year grid through the retained horizon, and the ACS 5-year tract series does not exist before `2010`.
  Date/Author: 2026-03-21 / Inder Majumdar + Codex
...
- Decision: Pause after the review-stage annual-ACS tract run and do not promote the sidecar artifacts into the canonical tract outputs until the tract-geography mismatch across ACS years is resolved.
  Rationale: The review-stage tract summaries showed small tract-ID drift beginning in `2011` and, before the later `2019` truncation, a large tract-universe mismatch in `2020`, so promotion at that stage would have silently baked an unresolved geography-harmonization problem into the main tract panel.
...
- Decision: Build tract waiver-count descriptives from `2_0_7_waived_data_consolidated_long_tract.rds` rather than from the tract analysis panel.
  Rationale: The tract panel inherits a legacy county-style override that forces `treatment = 1` in `2010:2013`, so the tract waiver source is the correct artifact for descriptive counts intended to reflect actual tract waiver timing.
  Date/Author: 2026-03-21 / Codex

- Decision: Close the current ExecPlan after the `2019`-bounded ingest branch and the two initial tract descriptive outputs, and move any broader tract descriptive interrogation to a separate future ExecPlan.
  Rationale: The user wants to pause the descriptive layer and treat further tract descriptives as a separate planning task rather than leaving this ingest-heavy ExecPlan open indefinitely.
  Date/Author: 2026-03-21 / Inder Majumdar + Codex
...
This ExecPlan completed the tract ingest branch and the initial tract descriptive handoff. Milestone 1 built the tract waiver crosswalk, tract waiver panel, tract retailer tract panel, and tract pre-covariate panel. Milestone 2 rebuilt the tract ACS sidecars as a `2000:2019` / `2010:2019` branch using direct tract income and rent controls plus an explicit 26-tract exclusion set. Milestone 3 was intentionally stopped after one tract waiver descriptive and one tract retailer descriptive output so the user can handle broader tract descriptive interrogation in a separate future ExecPlan.
...
- Expected outcome at this stage: the revised Milestone 2 should add year-specific tract ACS covariates for `2010:2019`, a revised tract analysis panel that uses those year-specific controls on the full `2000:2019` year grid, and summary diagnostics that make both annual join coverage and the documented `2010` backfill for years `2000:2009` explicit.
- Actual outcome: the review-stage annual-ACS sidecar artifacts now exist and meet that standard. They show zero non-excluded tract join loss in every retained ACS year `2010:2019`, document the explicit 26-tract exclusion set, and carry the benchmark county wage gap only in pre-2010 years. The sidecar filenames retain the `_2010_2020_` stems for continuity, but their current contents stop at `2019`.
- Difference (if any): the tract retailer branch still uses substantial nearest-tract fallback, so tract retailer descriptives should continue to be interpreted with that caveat. The descriptive layer is intentionally partial because the user chose to move further tract descriptive work into a separate future plan.
...
- Challenge: the first tract and county waiver-count descriptives were flat even though the underlying waiver counts varied over time.  
  Resolution: the plotting bug came from `count(..., wt = 1, ...)`; removing the constant weight fixed the counting logic, and the tract waiver descriptive was also redirected to the tract waiver source rather than the tract panel.
...
- Lesson: tract waiver descriptives should not infer waiver timing from the tract analysis panel without first checking whether the panel inherits legacy treatment-timing overrides from the county workflow.

**Follow-up Work**

- Follow-up task: open a separate future ExecPlan for any broader tract descriptive buildout or tract-descriptive auditing beyond the two initial outputs already created.
- Follow-up task: assess whether the retailer fallback share needs additional refinement before treating future tract retailer descriptives as final.
...
The third workstream adds tract controls and tract analysis panels. Revise `1_code/1_0_ingest/1_0_1_covariates/1_0_1_4_ACS_tract_prep.R` so it pulls tract ACS 5-year variables via `tidycensus` for every retained year `2010` through `2019`. The script must read the key from `0_inputs/census_apikey.md`, register it for the session without hardcoding it into script source, and then fetch the raw variables needed to build tract median earnings from `B20002_001E`, `urate`, `population`, and tract median gross rent from `B25064_001E` for each ACS year in that range. It should keep only the raw fields still needed for the chosen direct controls and any remaining derived control, record whether tract-level `DP03_0063E` is available for each year, construct a tract covariate file on the full `2000:2019` year grid, and explicitly fill years `2000:2009` with the `2010` ACS 5-year tract values because no tract ACS 5-year series exists before `2010`. The script should also write an explicit annual-ACS tract exclusion table based on the reviewed `2011:2019` mismatch set: `15` `94xx` tract-code cases (`36053940101`, `36053940102`, `36053940103`, `36053940200`, `36053940300`, `36053940401`, `36053940403`, `36053940600`, `36053940700`, `36065940000`, `36065940100`, `36065940200`, `46113940500`, `46113940800`, `46113940900`) and `11` other ACS non-returned tract IDs (`02270000100`, `04019002701`, `04019002903`, `04019410501`, `04019410502`, `04019410503`, `04019470400`, `04019470500`, `06037930401`, `36085008900`, and `51515050100`). The diagnostics should report both classes separately and cite the tract-code rationale documented in this ExecPlan. Then use `1_code/1_0_ingest/1_0_2_build_panel/1_0_2_1_build_analysis_panel_tract_pre_covariates.R` as the unchanged tract-year backbone that carries treatment timing and retailer outcomes on the full annual panel. Finally, revise `1_code/1_0_ingest/1_0_2_build_panel/1_0_2_2_build_analysis_panel_tract.R` so it merges the year-specific tract ACS controls onto matching tract-years for `2010:2019`, backfills tract ACS controls in years `2000:2009` with the `2010` ACS 5-year values, carries county-level wages alongside them, respects the explicit annual-ACS tract exclusion table, and writes review-stage annual-ACS sidecar panel artifacts and summaries for user inspection before any canonical overwrite.

The fourth workstream adds tract descriptive scripts only. Do not add any tract reduced-form scripts. Place the tract waiver descriptive companions in `1_code/1_1_descriptives/1_1_0_waivers/` beside the county waiver descriptives and the tract retailer descriptive companions in `1_code/1_1_descriptives/1_1_1_retailers/` beside the county retailer descriptives. Because output routing depends on the script folder, these placements will send tract waiver outputs to `3_outputs/3_1_descriptives/3_1_0_waivers/` and tract retailer outputs to `3_outputs/3_1_descriptives/3_1_1_retailers/`. Every tract output filename and every user-facing title or subtitle should include `_tract` so the tract artifacts can be singled out easily. This ExecPlan stops after the two initial tract descriptive outputs already created; any broader tract descriptive buildout should move to a separate future plan.

Execution was broken into the two user-requested tract ingest milestones before plotting work. Milestone 1 is the tract pre-covariate build: waiver-to-tract crosswalk, SNAP retailer-to-tract panel, and `2_9_3_us_analysis_panel_tract_pre_covariates.rds`, together with diagnostics proving that no in-scope waiver conferrals were dropped unexpectedly. Milestone 2 is the revised annual-ACS tract covariate pull plus the revised tract panel, together with diagnostics proving year-specific tract ACS join coverage for `2010:2019`, making any remaining source-data missingness explicit, documenting that years `2000:2009` are filled with the `2010` ACS 5-year tract values to preserve the county-style year grid, and reporting the reviewed tract-drop set explicitly rather than leaving those cases buried inside the unmatched count. This ExecPlan closes after those ingest milestones plus the two initial tract descriptive outputs already created.
...
3. Run Milestone 2.
   ...
   Expected behavior: the revised scripts write review-stage annual-ACS sidecar artifacts under the processed-data root: `2_1_8`, `2_1_9`, `2_1_10`, `2_9_6`, and `2_9_7`. The covariate summary should show year-specific tract ACS join coverage for `2010:2019`, should separate join coverage from source-value missingness, should report that years `2000:2009` were populated from the `2010` ACS 5-year tract values, should document how county-level wages were merged, and should list the explicit annual-ACS tract exclusions rather than leaving those rows only in aggregate unmatched counts.

4. Build one tract waiver descriptive output and one tract retailer descriptive output.

       /usr/local/bin/Rscript 1_code/1_1_descriptives/1_1_0_waivers/1_1_0_3_tract_conferral_growth_rural_share.R
...
1. Command:
       ...
       test -f 3_outputs/3_1_descriptives/3_1_0_waivers/3_1_0_3_tract_conferral_growth_rural_share.jpeg -o -f 3_outputs/3_1_descriptives/3_1_0_waivers/3_1_0_3_tract_conferral_growth_rural_share_v01.jpeg
...
- [x] Revised annual-ACS tract covariate pull and merge rerun with the explicit `2011:2019` tract exclusion table, `B20002_001E` tract income, `B25064_001E` tract rent, and a documented `2010` backfill used in years `2000:2009`
...
- [x] At least one tract waiver descriptive output and one tract retailer descriptive output exist in `3_outputs/3_1_descriptives/3_1_0_waivers/` and `3_outputs/3_1_descriptives/3_1_1_retailers/`, each clearly marked with `_tract`
- [x] County scripts and county outputs remain intact
- [x] `Progress`, `Surprises & Discoveries`, `Decision Log`, and `Outcomes & Retrospective` reflect the final state
...
- 2026-03-21: ExecPlan revised so Milestone 2 would be rerun with year-specific ACS 5-year tract covariates, a documented `2010` backfill for years `2000:2009` to preserve the county-style year grid, and review-stage sidecar artifacts for user inspection before any overwrite of the first-pass single-vintage outputs.
- 2026-03-21: The review-stage annual-ACS tract run was executed and produced `2_1_8`, `2_1_9`, `2_1_10`, `2_9_6`, and `2_9_7`. Those artifacts revealed tract-geography drift across ACS years, especially in `2020`, before the retained branch was later curtailed to `2019`.
...
- 2026-03-21: Revised Milestone 2 rerun executed. The refreshed annual-ACS sidecars now use `B20002_001E` for tract income, `B25064_001E` for tract rent, exclude the reviewed `26` tract IDs explicitly, and achieve zero non-excluded join loss in `2010:2019`.
- 2026-03-21: The tract Milestone 2 horizon was curtailed to end in `2019`, matching the current reduced-form sample window and removing the `2020` tract-redraw issue from the tract sidecars. Milestone 3 then produced one tract waiver descriptive output and one tract retailer descriptive output in the existing descriptive folders before broader descriptive work was deferred to a separate future plan.
