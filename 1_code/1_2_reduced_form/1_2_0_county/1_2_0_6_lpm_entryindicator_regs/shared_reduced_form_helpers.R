...
# Description:      Shared helper functions for the county reduced-form LPM
#                   entry-indicator scripts. These helpers resolve paths, load
#                   the entry-indicator event-study sample, estimate the shared
#                   Sun-Abraham specification in levels, and write versioned
...
# Load the saved county LPM entry-indicator sample used by the helper-driven
# scripts.
# Purpose: separate sample construction from repeated estimation scripts.
# Inputs: none.
# Returns: the `2_9_9_event_study_sample_lpm_entryindicator.rds` data frame.
...
load_event_study_sample <- function() {
  ...
  readRDS(file.path(processed_root, "2_9_analysis", "2_9_9_event_study_sample_lpm_entryindicator.rds"))
}

# These registries define the entry-indicator outcome order and display labels
# used by the single-outcome scripts, combined ATT table, and slide-ready image
# table.
event_study_outcomes <- c(
  "total_ds_entry",
  "chain_super_market_entry",
  "chain_convenience_store_entry",
  "chain_multi_category_entry",
  "chain_medium_grocery_entry",
  "chain_small_grocery_entry",
  "chain_produce_entry",
  "chain_farmers_market_entry"
)
...
# Estimate the county LPM reduced form for one outlet-entry outcome.
# Purpose: keep the county LPM Sun-Abraham specification identical across
#          outcome scripts.
...
# Review focus: confirm the indicator-in-levels outcome, covariate set,
#               event-study term, and county clustering.
run_event_study_model <- function(var_name) {
  aba1 <- load_event_study_sample()

  if (!var_name %in% names(aba1)) {
    stop(sprintf("Outcome '%s' is not present in the county LPM entry-indicator sample.", var_name))
  }
...
# Estimate the county LPM reduced form and save its figure and ATT table.
...
save_event_study_artifact <- function(var_name, label, file_stub) {
  ...
  event_plot <- ggplot(
    ...
    labs(
      title = paste0(label, " Entry Indicator"),
      x = "Event time",
      y = "Coefficient"
    ) +
