...
# Description:      Shared helper functions for the census-tract nonlinear
#                   reduced-form branch. These helpers resolve repository
#                   paths, load the tract binary DID sample, bridge from R into
#                   Python `diff_diff.WooldridgeDiD`, and write versioned
#                   figure/table/csv artifacts under `3_outputs`.
# INPUTS:           `2_processed_data/processed_root.txt`
#                   `2_9_analysis/2_9_8_censustract_logit_sample.rds`
# OUTPUTS:          No direct outputs. Downstream scripts use these helpers to
#                   write versioned `.pdf`, `.tex`, and `.csv` artifacts under
#                   `3_outputs`.
# DEPENDENCIES:     `dplyr`, `ggplot2`, `reticulate`
# Review focus:     Verify that the Python bridge targets
#                   `diff_diff.WooldridgeDiD(method = "logit")` with
#                   `control_group = "never_treated"` and county-clustered
#                   standard errors, while continuing to prevent silent output
#                   overwrites by versioning every artifact path.
#///////////////////////////////////////////////////////////////////////////////
...
ensure_reduced_form_dirs <- function() {
  plot_dir <- file.path("3_outputs", "3_2_reduced_form", "3_2_1_censustract")
  table_dir <- file.path("3_outputs", "3_0_tables", "3_2_1_censustract")
...
tract_binary_control_group <- "never_treated"
tract_binary_covariates <- c("population", "wage", "meanInc", "rent", "urate")
tract_binary_method <- Sys.getenv("SNAP_DOLLAR_ENTRY_TRACT_BINARY_METHOD", unset = "logit")

validate_binary_method <- function(method = tract_binary_method) {
  if (identical(method, "logit")) {
    return(method)
  }

  stop(
    paste0(
      "Unsupported census-tract binary DID method '", method, "'. ",
      "The installed diff-diff WooldridgeDiD implementation documents ",
      "only {'ols', 'logit', 'poisson'}, so probit is not available here."
    )
  )
}

control_group_display_label <- function(control_group = tract_binary_control_group) {
  switch(
    control_group,
...
load_tract_logit_sample <- function() {
  repo_root <- get_repo_root()
  setwd(repo_root)

  processed_root <- read_root_path("2_processed_data/processed_root.txt")
  readRDS(file.path(processed_root, "2_9_analysis", "2_9_8_censustract_logit_sample.rds"))
}
...
build_logit_model_frame <- function(var_name) {
  sample <- load_tract_logit_sample()

  if (!var_name %in% names(sample)) {
    stop(sprintf("Outcome '%s' is not present in the census-tract logit sample.", var_name))
  }
  ...
  model_frame <- sample |>
    transmute(
      ...
      outcome = as.integer(.data[[var_name]]),
    ...
    filter(
      ...
      !is.na(outcome),
      outcome %in% c(0L, 1L),
      if_all(all_of(tract_binary_covariates), is.finite)
    )
...
run_event_study_model <- function(var_name) {
  ...
  method <- validate_binary_method()
  model_frame <- build_logit_model_frame(var_name)

  estimator <- diff_diff$WooldridgeDiD(
    method = method,
    control_group = tract_binary_control_group,
    cluster = "county_fips",
    alpha = 0.05
  )
  ...
  if (length(tract_binary_covariates) > 0) {
    fit_args$xtvar <- tract_binary_covariates
  }
...
build_event_plot <- function(event_df, label, method) {
  if (nrow(event_df) == 0) {
  ...
  ggplot(event_df, aes(x = relative_period, y = att)) +
    ...
    labs(
      title = paste0("Census-Tract Binary ETWFE ATT: ", label),
      subtitle = paste0(
        "diff-diff WooldridgeDiD(method = ",
        method,
        ") with ",
...
write_single_outcome_table <- function(simple_df, label, method, output_path) {
  row <- simple_df[1, ]
  ...
  table_lines <- c(
    ...
    "   \\midrule \\midrule",
    sprintf(
      "   \\multicolumn{6}{l}{\\emph{Estimator: diff-diff WooldridgeDiD(method = %s)}}\\\\",
      method
    ),
    sprintf(
...
save_event_study_artifact <- function(var_name, label, file_stub) {
  ...
  event_plot <- build_event_plot(outputs$event_df, label, model_bundle$method)
  ggsave(plot_path, event_plot, width = 8, height = 6, units = "in")

  write_single_outcome_table(outputs$simple_df, label, model_bundle$method, tex_path)
