#///////////////////////////////////////////////////////////////////////////////
#----                              Preamble                                 ----
# File name:        1_1_2_1b_weighted_distance_snapshot_table_evertreated.R
# Previous author:  -
# Current author:   Codex
# Last Updated:     May 27, 2026
# Description:      Build the weighted-distance snapshot table for `2014`,
#                   split by ever-treated versus never-treated counties and
#                   faceted by urban and rural counties.
# INPUTS:           `2_10_retail_access/2_10_2_county_retail_access_weighted_summary.rds`
# OUTPUTS:          `3_outputs/3_1_descriptives/3_1_2_retail_access/3_1_2_1b_weighted_distance_snapshot_table_evertreated.csv`
#                   `3_outputs/3_1_descriptives/3_1_2_retail_access/3_1_2_1b_weighted_distance_snapshot_table_evertreated.jpeg`
#///////////////////////////////////////////////////////////////////////////////
...
treatment_levels <- c("Ever-treated counties", "Never-treated counties")

snapshot_table <- ctx$county_summary |>
  filter(year == 2014L) |>
  group_by(rural_status, treatment_group, format, weight_type) |>
  summarise(
  ...
  mutate(
    ...
    treatment_group = factor(as.character(treatment_group), levels = treatment_levels),
    format = factor(format, levels = ctx$format_levels),
    weight_type = factor(weight_type, levels = ctx$weight_levels)
  ) |>
  arrange(rural_status, format, weight_type, treatment_group)

readr::write_csv(
  snapshot_table,
  retail_access_output_path("3_1_2_1b_weighted_distance_snapshot_table_evertreated.csv")
)
...
p <- ggplot(
  table_plot_data,
  aes(x = treatment_group, y = row_key)
) +
  ...
  labs(
    ...
    subtitle = "Mean county weighted distance in miles, using 2014 and splitting counties by ever-treated status.",
    x = NULL,
    y = NULL
  ) +
...
ggsave(
  filename = retail_access_output_path("3_1_2_1b_weighted_distance_snapshot_table_evertreated.jpeg"),
  plot = p,
  width = 10,
  height = 9,
  units = "in"
